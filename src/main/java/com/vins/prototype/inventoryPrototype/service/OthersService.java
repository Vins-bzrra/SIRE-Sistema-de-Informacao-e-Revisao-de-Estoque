...
import com.vins.prototype.inventoryPrototype.dto.OthersItemsDto;
...
public class OthersService {
	@Autowired
	private OthersItemsRepository repository;
	...
	public void createItem(OthersItemsDto item) {
		try {
			OthersItems othersItems = new OthersItems();

			othersItems.setBrand(item.getBrand());
			othersItems.setModel(item.getModel());
			othersItems.setAcquisitionDate(item.getAcquisitionDate());
			othersItems.setSupplier(item.getSupplier());
			othersItems.setUnitLocation(item.getUnitLocation());
			othersItems.setStatus(item.getStatus());
			...
			othersItems.setPatrimony(item.getPatrimony());
			othersItems.setPurchasePrice(item.getPurchasePrice());
			othersItems.setCurrentOwner(item.getCurrentOwner());
			othersItems.setCategory(item.getCategory());
			othersItems.setDescription(item.getDescription());
			othersItems.setProcessNumber(item.getProcessNumber());
			othersItems.setMaterial(item.getMaterial());
			othersItems.setDimensions(item.getDimensions());
			othersItems.setColor(item.getColor());
			othersItems.setSerialNumber(item.getSerialNumber());

			repository.save(othersItems);
		} catch (Exception e) {
	...
	public HttpStatus transfer(ItemTransferDto transfer, User user) {
		try {
			...
			Optional<OthersItems> optionalItem = repository.findByPatrimony(patrimony);
			if (optionalItem.isEmpty()) {
				throw new RuntimeException("Item não localizado!");
			}

			OthersItems item = optionalItem.get();
	...
	public List<OthersItems> search(OthersItemsDto search) {
		try {
			Specification<OthersItems> specification = (root, query, criteriaBuilder) -> {
	...
	public OthersItems getItem(Long id) {

		return repository.findById(id).orElse(null);
	}

	public void updateItem(OthersItemsDto itemDto, User user) {
		try {
			Optional<OthersItems> optionalItem = repository.findById(itemDto.getId());
			if (optionalItem.isPresent()) {
				...
				if (itemDto.getBrand() != null && !itemDto.getBrand().isEmpty()) {
					itemService.registerItemChange("brand", item.getBrand(), itemDto.getBrand(), item.getPatrimony(),
							user);
					item.setBrand(itemDto.getBrand());
				}
				if (itemDto.getModel() != null && !itemDto.getModel().isEmpty()) {
					itemService.registerItemChange("model", item.getModel(), itemDto.getModel(), item.getPatrimony(),
							user);
				...
				if (itemDto.getSupplier() != null && !itemDto.getSupplier().isEmpty()) {
					itemService.registerItemChange("supplier", item.getSupplier(), itemDto.getSupplier(),
				...
				if (itemDto.getUnitLocation() != null && !itemDto.getUnitLocation().isEmpty()) {
					itemService.registerItemChange("unitLocation", item.getUnitLocation(), itemDto.getUnitLocation(),
							item.getPatrimony(), user);
					item.setUnitLocation(itemDto.getUnitLocation());
				}
				if (itemDto.getStatus() != null && !itemDto.getStatus().isEmpty()) {
					itemService.registerItemChange("status", item.getStatus(), itemDto.getStatus(), item.getPatrimony(),
							user);
				...
				if (itemDto.getCurrentOwner() != null && !itemDto.getCurrentOwner().isEmpty()) {
					itemService.registerItemChange("currentOwner", item.getCurrentOwner(), itemDto.getCurrentOwner(),
							item.getPatrimony(), user);
					item.setCurrentOwner(itemDto.getCurrentOwner());
				}
				if (itemDto.getCategory() != null && !itemDto.getCategory().isEmpty()) {
					itemService.registerItemChange("category", item.getCategory(), itemDto.getCategory(),
							item.getPatrimony(), user);
					item.setCategory(itemDto.getCategory());
				}
				if (itemDto.getDescription() != null && !itemDto.getDescription().isEmpty()) {
					itemService.registerItemChange("Description", item.getDescription(), itemDto.getDescription(),
							item.getPatrimony(), user);
				...
				if (itemDto.getMaterial() != null && !itemDto.getMaterial().isEmpty()) {
					itemService.registerItemChange("material", item.getMaterial(), itemDto.getMaterial(),
							item.getPatrimony(), user);
					item.setMaterial(itemDto.getMaterial());
				}
				if (itemDto.getDimensions() != null && !itemDto.getDimensions().isEmpty()) {
					itemService.registerItemChange("dimensions", item.getDimensions(), itemDto.getDimensions(),
							item.getPatrimony(), user);
					item.setDimensions(itemDto.getDimensions());
				}
				if (itemDto.getColor() != null && !itemDto.getColor().isEmpty()) {
					itemService.registerItemChange("color", item.getColor(), itemDto.getColor(), item.getPatrimony(),
							user);
					item.setColor(itemDto.getColor());
				}
				if (itemDto.getSerialNumber() != null && !itemDto.getSerialNumber().isEmpty()) {
					itemService.registerItemChange("serialNumber", item.getModel(), itemDto.getModel(),
							item.getPatrimony(), user);
					item.setSerialNumber(itemDto.getSerialNumber());
				}
				if (itemDto.getPatrimony() != null && !itemDto.getPatrimony().isEmpty()
						&& !itemDto.getPatrimony().equals(item.getPatrimony())) {
					itemService.registerItemChange("patrimony", item.getPatrimony(), itemDto.getPatrimony(),
							item.getPatrimony(), user);
